                       ...
                       <div key={index} className="bg-navy p-6 rounded-lg border border-slate-dark/30 hover:border-accent-yellow/50 transition-all duration-300 transform hover:-translate-y-1 flex flex-col justify-between">
                           <div>
                               <div className="flex justify-between items-start mb-3">
                                   ...
                                   <span className="text-xs font-medium bg-slate-dark/50 text-accent-cyan px-2 py-1 rounded">{course.credits}</span>
                               </div>
                               <div className="flex items-center space-x-4 text-sm text-slate mb-4">
                                   <span>{course.lecturer}</span>
                                   <span className="flex items-center"><Star className="w-4 h-4 text-accent-yellow mr-1" /> {course.rating}</span>
                               </div>
                            ...
                            <button disabled={course.registered} className={`w-full py-2.5 rounded-md text-sm font-semibold transition-all ${course.registered ? 'bg-slate-dark/50 text-slate cursor-not-allowed' : 'bg-accent-yellow text-navy-dark hover:bg-accent-yellow-dark'}`}>
                       ...
                       {curriculum.map((item, index) => (
                           <li key={item.code} className="p-4 flex justify-between items-center hover:bg-slate-dark/20 transition-colors">
                               <div className="flex items-center">
                                   <span className="text-slate mr-4 font-mono">{index + 1}.</span>
                                   <div>
                               ...
                               <span className={`text-xs font-bold ${item.completed ? 'text-accent-cyan' : 'text-slate'}`}>
