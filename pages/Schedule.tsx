...
const Schedule: React.FC = () => {
 return (
   <div className="animate-page-transition flex flex-col h-full space-y-8">
     ...
     <div className="grid grid-cols-7 gap-4 flex-grow">
       {weeklySchedule.map((day) => (
         <div key={day.day} className="bg-navy p-3 rounded-lg flex flex-col space-y-3">
           ...
           <div className="space-y-3 flex-grow">
             {day.classes.length > 0 ? (
               day.classes.map((cls, index) => (
                 <div key={index} className={`p-3 rounded-md border-l-4 ${cls.color}`}>
               ...
               <div className="h-full flex items-center justify-center">
                   <p className="text-slate/60 italic text-xs text-center">Không có lớp</p>
               </div>
             )}
           </div>
         </div>
     ...
     <div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
         {upcomingEvents.map((event, index) => (
           <div key={index} className="bg-navy p-4 rounded-lg border border-slate-dark/30 hover:border-accent-yellow/50 transition-colors flex items-start space-x-4">
               <div className={`p-2 bg-slate-dark/30 rounded-md`}>
                   <event.icon className={`w-6 h-6 ${event.color}`} />
               </div>
               <div>
                   <h4 className="font-semibold text-slate-lightest leading-tight">{event.title}</h4>
